...
def list_tools():
    print(bcolors.OKBLUE + "Available Tools:" + bcolors.ENDC)
    print("-----------------")
    for i, tool in enumerate(tool_names, start=1):
        print(f"{i}. {tool[0]} - {tool[1]}")
    print()

# Web scanner Help Context
def helper():
    print(bcolors.OKBLUE + "Information:" + bcolors.ENDC)
    print("------------")
    print("\t./cloudx.py example.com: Scans the domain example.com.")
    print("\t./cloudx.py example.com --skip dmitry --skip theHarvester: Skip the 'dmitry' and 'theHarvester' tests.")
    print("\t./cloudx.py example.com --nospinner: Disable the idle loader/spinner.")
    print("\t./cloudx.py --update   : Updates the scanner to the latest version.")
    print("\t./cloudx.py --help     : Displays this help context.")
    print("\t./cloudx.py --list     : Lists all available tools.")
    print(bcolors.OKBLUE + "Interactive:" + bcolors.ENDC)
    print("------------")
    print("\tCtrl+C: Skips current test.")
    print("\tCtrl+Z: Quits CloudX.")
...
def main():
    parser = argparse.ArgumentParser(description='CloudX - Web Scanner')
    parser.add_argument('url', metavar='URL', type=str, help='The URL to be scanned.')
    parser.add_argument('--skip', metavar='SKIP', type=str, nargs='+', help='Tools to be skipped.')
    parser.add_argument('--nospinner', action='store_true', help='Disable the idle loader/spinner.')
    parser.add_argument('--update', action='store_true', help='Updates the scanner to the latest version.')
    parser.add_argument('--list', action='store_true', help='Lists all available tools.')
...
class Spinner:
    ...
    def spinner_task(self):
        ...
        try:
            ...
            print("\n\t" + bcolors.BG_ERR_TXT + "CloudX received a series of Ctrl+C hits. Quitting..." + bcolors.ENDC)
            sys.exit(1)
...
tool_cmd = [
    ...
    # 2
    ["wget -O /tmp/cloudx_temp_aspnet_config_err --tries=1 ", "/%7C~.aspx"],

    # 3
    ["wget -O /tmp/cloudx_temp_wp_check --tries=1 ", "/wp-admin"],

    # 4
    ["wget -O /tmp/cloudx_temp_drp_check --tries=1 ", "/user"],

    # 5
    ["wget -O /tmp/cloudx_temp_joom_check --tries=1 ", "/administrator"],
    ...
    # 75
    ["wget -O /tmp/cloudx_temp_aspnet_elmah_axd --tries=1 ", "/elmah.axd"],
    ...
    # 78
    ["wapiti ", " -f txt -o cloudx_temp_wapiti"],
...
if args_namespace.help or (not args_namespace.update
    ...
    os.system(
        'wget -N https://raw.githubusercontent.com/MAPLEIZER/Cloud_X/main/cloudx.py -O cloudx.py > /dev/null 2>&1')  # S://github.com/MAPLEIZER/Cloud_X.git
    ...
    if oldversion_hash == newversion_hash:
        clear()
        print("\t" + bcolors.OKBLUE + "You already have the latest version of CloudX." + bcolors.ENDC)
    else:
        clear()
        print("\t" + bcolors.OKGREEN + "CloudX successfully updated to the latest version." + bcolors.ENDC)
    spinner.stop()
    sys.exit(1)
    ...
    while (rs_avail_tools < len(tools_precheck)):
        ...
        try:
            ...
            print("\t" + bcolors.BG_ERR_TXT + "CloudX was terminated abruptly..." + bcolors.ENDC)
            sys.exit(1)
    ...
    if len(unavail_tools_names) == 0:
        print(
            "\t" + bcolors.OKGREEN + "All Scanning Tools are available. Complete vulnerability checks will be performed by CloudX." + bcolors.ENDC)
    else:
        print("\t" + bcolors.WARNING + "Some of these tools " + bcolors.BADFAIL + str(
            unavail_tools_names) + bcolors.ENDC + bcolors.WARNING + " are unavailable or will be skipped. CloudX will still perform the rest of the tests. Install these tools to fully utilize the functionality of CloudX." + bcolors.ENDC)
    ...
    while (tool < len(tool_names)):
        ...
        temp_file = "/tmp/cloudx_temp_" + tool_names[tool][arg1]
        ...
        if runTest == 1:
            ...
            print(bcolors.OKBLUE + "\nScan Interrupted in " + display_time(int(elapsed)) + bcolors.ENDC, end='\r',
                  flush=True)
            print(
                "\n" + bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit CloudX.\n" + bcolors.ENDC)
            rs_skipped_checks = rs_skipped_checks + 1
    ...
    if len(rs_vul_list) == 0:
        ...
        with open(vulreport, "a") as report:
            while (rs_vul < len(rs_vul_list)):
                ...
                report.write("\n------------------------\n\n")
                temp_report_name = "/tmp/cloudx_temp_" + vuln_info[arg1]
            ...
            print(
                "\tComplete Vulnerability Report for " + bcolors.OKBLUE + target + bcolors.ENDC + " named " + bcolors.OKGREEN + vulreport + bcolors.ENDC + " is available under the same directory CloudX resides.")
    ...
    for file_index, file_name in enumerate(tool_names):
        with open(debuglog, "a") as report:
            try:
                with open("/tmp/cloudx_temp_" + file_name[arg1], 'r') as temp_report:
                    data = temp_report.read()
    ...
    print("\n")
    print(
        "\tFor Debugging Purposes, You can view the complete output generated by all the tools named " + bcolors.OKBLUE + debuglog + bcolors.ENDC + " under the same directory.")
